...
Page({
  ...
  // 试色h5
  testColor() {
    let url = 'https://2018cvday.yslbeautycn.com/platform/index.html/';
  ...
  getDetail: function(options) {
    ...
    let uinfo = app.getCache('userinfo_id');
    core.get('goods/get_detail', {
      ...
      $this.setData({
        ...
        view:result.goods.view,
        viewstatus:result.goods.view_status
      });
  ...
  onLoad: function(options) {
    ...
    // core.get('goods/get_picker', { id: options.id, optionid: "" }, function (res) {
    //   console.log('res', res, res.morenSel[0].id, res.specs[0].items)
    //   let newid = res.morenSel[0].id;
    //      if(res.error == 0){
    //        for(let i = 0 ; i <  res.specs[0].items.length;i++){
    //          if (res.specs[0].items[i].id == newid){
    //            console.log('dsssssssssssssss')
    //            wx.setStorageSync("linshiguige", [{ id: res.specs[0].items[i].optionid}])
    //          }
    //        }
    //      }
    //  })
  ...
  onShareAppMessage: function() {
    ...
    return core.onShareAppMessage('/pages/goods/detail/index?id=' + this.data.goods.id, this.data.goods.title);
  },
  showpic: function() {
    this.setData({
      showpic: true,
  ...
  toggleLike:function(e){
    ...
    let uid = app.getCache('userinfo_id')
    if(dataType == 'like'){
      let data = {
        'goodsid': goodsid,
        'uid': uid
